...
public class DingdanImpl implements DingdanDao {
	...
	public int addDingdan(Dingdan dingdan) throws Exception {
		Connection conn = DBUtils.getConnection();
                String sql = "insert into dingdan(shopid,goodsid) value(?,?)";
		...
		System.out.println(dingdan.getShopid());
                ps.setInt(1, dingdan.getShopid());
		ps.setInt(2, dingdan.getGoodsid());
	...
	public List<Dingdan> findDingdan(Users users) throws Exception {
		...
		String sql = " SELECT  dingdan.dingdanstatus,dingdan.goodsname,goodsid,dingdan.count,goodsprice,shop.userid FROM dingdan,shop,goods "
                ...
                /*String sql = " SELECT  dingdan.dingdanstatus,goods.goodsname,goodsid,goods.count,goodsprice,shop.userid FROM dingdan,shop,goods "
				+ "WHERE shop.id=dingdan.shopid and dingdan.goodsid=goods.id "
				+ "and dingdanstatus=0 and goods.userid=?";*/
		PreparedStatement ps = conn.prepareStatement(sql);
	...
	public List<Dingdan> findAllDingdan(int page, int number) throws Exception {
		...
		Connection conn = DBUtils.getConnection();
		String sql = "select dingdan.dingdanstatus,dingdan.goodsname,goodsid,dingdan.count,goodsprice,shop.userid FROM dingdan,shop,goods"
                        + "from dingdan,shop,goods"
                        + " where shop.id=dingdan.shopid and dingdan.goodsid=goods.id "
                        + " and dingdanstatus=0 and goods.userid=? limit ?,?";
		PreparedStatement ps = conn.prepareStatement(sql);
                
                ps.setInt(1, number);
		ps.setInt(2, (page - 1) * number);
		ps.setInt(3, number);
		...
		while (rs.next()) {
			...
			dingdan3.setId(rs.getInt("id"));
			dingdan3.setGoodsid(rs.getInt("goodsid"));
                        dingdan3.setId(rs.getInt("shop.userid"));
			list.add(dingdan3);
		}
		return list;
	}*/
